...
画面の確認は、そのプロダクトが実際に描いている表現で行う。セルで描く UI をビットマップに変換しない。情報が落ち、読む側のトークンも食う。

- **セル／文字の UI**（端末、TUI）: テストバックエンドのバッファを文字列として dump し、そのテキストで判断する。スクリーンショットや画面録画にしない。プロジェクトに dump 手順があればそれに従う。
- **ピクセルの UI**（通常の Web ページなど）: 実画面を見る。devtools が使えるなら関係画面を渡り、変更対象と遷移元・遷移先のレイアウト・間隔・色を目で確認する。

どちらの場合も、コードだけ読んで見た目を推測して終わらせない。確認手段が無いときだけ、実画面を見られていないことを報告に書く。「手段がない」とはツールが存在しない場合を指す。手間がかかるだけなら手段はある。
...
2. **動作確認** - テストが通ることと動くことは別。実際に動かす
   - UI 変更: Phase 2 と同じ表現で確認する。セル／文字の UI はバッファの文字列 dump を見比べる。ピクセルの UI は実画面を見比べる。同じコンポーネントを使う関連画面と、レイアウトが幅で変わるなら複数幅も見る
