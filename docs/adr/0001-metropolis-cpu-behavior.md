...
### 2. enumerate_actions の Demolish 候補は **すべての Built タイル** (= 統一)

Demolish 候補に対する per-Building の状況依存フィルタは置かない:

- 「active な Shop は守る」 → 全 Shop が active になったフェーズで足かせ
- 「Cottage 以外の House は守る」 → 全 House が Apartment になったフェーズで足かせ
- 「Plaza / Stadium はランドマーク」 → 終盤の再開発判断を奪う

これらはどれも「今この建物は健全だから」という状況依存の決め打ちで、街がそのフェーズを
**抜けた瞬間**に同じ問題 (= 撤去候補が枯れて再開発が止まる) を再生産する。次の Tier が
来るたびにフィルタを書き換える運用では、ロジックが状況に追随できない。

統一原則は **「列挙は無条件、判断は `action_value`」**:

- 列挙される Demolish 候補は「Built タイルかつ cash + reserve を満たす」全て
- 撤去するかどうかは `action_value = Δevaluate − cost` が判定する
- 健全な Highrise を撤去する手は Δevaluate が大きく負になり、search が自動的に却下する
  (= ロジック側で禁止する必要が無い)

唯一かける制約は **affordability** (cash + reserve)。これはどの建物にも同じ式で当たる、
状況非依存の純粋な物理制約。
...
- **候補数の線形増加**: Demolish 候補が「Built タイル全て」になり、Build 候補も
  per-Building の前提条件を撤廃したため、saturated map では候補が数百〜数千個。
  `rank_actions` は top_k 適用前に **全候補で `with_action_applied → eval_fn → 巻き戻し`**
  を実行するので、コストは O(候補数 × evaluate)。beam 幅で削れるのは深さ方向だけで、
  列挙コスト自体は削れない。
- **対応の方向**: `rank_actions` 内で baseline `before` 共有 (実装済み、~2x 高速化)
  に加え、(a) `evaluate` の差分計算 (action 適用セル + 影響半径だけ income 再計算)、
  (b) 前 tick の per-cell 評価 cache、(c) terminal な affordability 失敗の早期 skip、
  などが余地。本 ADR の milestones で順次扱う。
- **Tier 1 (Random) のキャラクター**: 状況フィルタが消えるので、ランダム Bot は
  健全な建物を破壊する確率が上がる。これは「Tier 1 はダメな思考」の表現として
  consistent だが、`is_game_progressing` の bar はそれを許容する形に緩めた。
- **evaluate の forecast 化**: 数式設計コストが大きい (バランス調整は dwell time
  と income table に依存)
...
- [x] `enumerate_actions` の per-Building Demolish フィルタを撤廃 (Built タイル
      なら全て候補。判断は `action_value` に委ねる)
- [x] `action_passes_guards` の per-Building house-count gate と savings protection
      を撤廃 (Build 候補も affordability のみで列挙)
- [x] `tick_observed` を logic.rs に追加し、`drive_ai` を no-op observer 経由で
      呼ぶ単一実装に統合 (本番経路と観測経路の挙動乖離リスクを構造的に排除)
...
- [ ] `evaluate` に forecast 価値項を加算 (Cottage の Apartment 化期待値、Road 接続
      期待効果、Outpost 解禁期待効果)。これが入ると Build 側の house-count gate が
      担保していた「序盤に空地で誤って高評価される」が真に不要になる
- [ ] `with_action_applied` 内の差分 evaluate (action 適用セル + 半径 5 だけ
      income 再計算) で saturated map の AI tick 時間を圧縮
- [ ] 診断テスト (`diagnose_t4_3h`) の停滞窓 / 怪しい手の閾値 assertion を入れる
