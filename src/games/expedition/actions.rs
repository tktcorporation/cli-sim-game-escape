...
pub const DROP_LANE_BASE: u16 = 50;
...
pub fn drop_lane_id(lane: u8) -> u16 {
    DROP_LANE_BASE + u16::from(lane)
}

pub fn lane_from_drop(action_id: u16) -> Option<u8> {
    if (DROP_LANE_BASE..DROP_LANE_BASE + 8).contains(&action_id) {
        Some((action_id - DROP_LANE_BASE) as u8)
    } else {
        None
    }
}
