...
///
/// **64×32 マップ対応**: 旧 32×16 では radius=5 (= ~60 セルの初期エリア)
/// だったが、4 倍の面積になったので radius=8 に拡張 (= ~145 セルの初期エリア)。
/// プレイヤーが詰まらないよう「最初の街区」を広めに確保する。
pub const CORE_RADIUS: u32 = 8;
...
///
/// **64×32 マップ対応**: CORE_RADIUS=8 に拡張したため、Rock 帯の入口を
/// d=9 から始めて、d=10 以上で徐々に高密度。Manhattan 距離の最大値は
/// `(GRID_W + GRID_H) / 2` ≈ 48 まで届くが、d ≥ 16 で 95% 飽和。
/// 拡張可能エリアを確保しつつ、外周の「岩壁感」を維持する。
///
/// バランス設計:
///   - d ≤ CORE_RADIUS (8): 0% — 中央コアは岩無し。
///   - d = 9: 25%   ← 境界の壁感を出す入り口
///   - d = 10: 45%
///   - d = 11: 60%
///   - d = 12: 75%
...
///   - d ≥ 14: 95%
///
/// d=9 から d=14 まで 6 段階のグラデーションで、Outpost 拡張がじわじわ
/// 進む感を作る。
...
mod tests {
    ...
    /// 境界 (Manhattan 距離 ≥ 14) では Rock が高密度 (8 シードの平均で 80% 以上)。
    ...
    fn rock_is_dense_at_boundary() {
        ...
        for s in 0..8u64 {
            ...
            // 外周帯にセルがあることを保証 (64×32 では d=14 以上のセルが
            // 余裕で存在するが、保険)。
        }
