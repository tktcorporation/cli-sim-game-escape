...
これは設計の全制約に優先する。具体的な閾値:

- 任意の連続 60 秒で **何か新しい建物が完成する** (built カウンタが +1 以上)。
- 任意の連続 60 秒で **同じセルに対する Build/Demolish/Replace が 3 回以上発生しない** (振動禁止)。
- 任意の時点で AI が `Idle` を選ぶ場合、**cash < $500 または free_workers == 0** のいずれかを満たす
  (= 「cash があるのに Idle」は禁止)。
...
| フィールド | 変更 |
| --- | --- |
| `last_build_finished_tick` | **新規**。`u64`。`advance_construction` で build/clearing 完了時に `city.tick` を記録 |
| `stagnation_started_tick` | **新規**。`Option<u64>`。stagnation 開始 tick |

### 4.4 simulator.rs

| テスト | 変更 |
| --- | --- |
| `no_stagnation_window_for_tier3_30min` | **新規**。30min 内で 60 sec 連続して built が増えない窓が無い |
| `no_oscillation_at_same_cell_tier4_30min` | **新規**。同セルに対する Build/Demolish/Replace が 60 sec 内に 3 回未満 |
| `no_idle_with_excess_cash_tier4_30min` | **新規**。`Idle with cash >= $2000` の発生率が全 actions の **5% 未満** |
| `ai_eval_cost_under_threshold` | **新規** (オプション)。Tier 4 の 1 tick AI 評価コストが基準値以下 |
...
- `last_build_finished_tick` / `stagnation_started_tick` を追加
...
- `apply_ai_action` / `with_action_applied` / `start_construction` で Replace 時 `built_at_tick` 継承
- → 「Replace で aging をリセットしない」コミット

### Step 6: stage bias (logic.rs)
- `cheap_action_score()` に `city_tier_for(population)` ベースの重み調整を追加
- → 「stage bias で Tier 4 が街の段階に合わせて建てる」コミット

### Step 7: stagnation breaker (ai.rs)
- `decide()` 入口で stagnation 検知 → `stagnation_breaker()` に分岐
- → 「停滞検知時の救済モード」コミット

### Step 8: 新規不変条件テストを ignore 解除
- 旧コードで先に書いておいた `no_stagnation_window_*` などを実行
- 全部 pass を確認
- → 「停滞回避を回帰テストで保証」コミット
...
```
を実行。warning が出たら次に進まない。

---

## 6. 期待される結果と検証

### 6.1 シミュレータ目標値 (Tier 4 / 30min / Income / seed=0xC1A5_5EED)

| 指標 | 旧 | 新目標 |
| --- | --- | --- |
| 60 秒以上 built 増えない窓 | あり (0–600 sec) | **なし** |
| 同セル 3 回振動 | あり (1200–1800 sec) | **なし** |
| Idle with cash >= $2000 | 2738/11714 (23%) | **< 5%** |
| 30min pop | 1300 | 500–1300 (緩い目標) |
| 30min cash | $75K | $5K–$80K (絶対値より「振動しない」が重要) |
| 30min built | 174 (頭打ち) | **継続的に増加** |
