...
| フィールド | 変更 |
| --- | --- |
| `last_progress_tick` | **新規**。`u64`。着工 (`start_construction`) / 撤去 (`demolish_at`) / 完成・整地完了 (`advance_construction`) の進捗イベントで `city.tick` を記録 |
...
| 指標 | 旧 | 新目標 |
| --- | --- | --- |
| 60 分以上 進捗イベント (着工/撤去/完成) ゼロ + Construction 空 の窓 | (ベンチ期間外なので未測定) | **なし** |
| 同セル 3 回振動 (60s) | あり (1200–1800 sec) | **なし** |
| Idle with cash >= $2000 | 2738/11714 (23%) | **< 5%** |
| 30min pop | 1300 | 100–1300 (緩い目標、ゆっくりで OK) |
| 30min cash | $75K | $5K–$80K (絶対値より「振動しない」が重要) |
| 30min built | 174 (頭打ち) | **継続的に増加または Construction 中** |
