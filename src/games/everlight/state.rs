...
pub struct EverlightState {
    ...
    /// render.rs は常時光輪の周回リング (現在位置・常に同じ暗さ) とは別に、
    /// これが有効な間だけ明るいパルスリングを重ねて描く。
    ///
    /// 常時リングの色そのものを切り替える設計にしなかった理由: 光輪の
    /// クールダウン (`OwnedWeapon::cooldown_ticks`) は最短5tickで、進化や
    /// 速射パッシブで短くなることはあってもこのフラッシュの持続tick数
    /// (`HALO_FLASH_TICKS`、まとめtick処理での見逃し防止のため5以上必要)
    /// を上回ることは無い。そのため常時リングの色を切り替える設計だと、
    /// 次の発火が必ず前のフラッシュが消える前後に来て「常時明るいまま」
    /// になり、パルスの意味が失われる。常時リングとパルスを別レイヤーに
    /// 分離することで、パルス頻度に関わらず常時リングは一貫した見た目を保つ。
    pub halo_flash: FlashTimer,
    /// `halo_flash` を立てた瞬間の判定位置 (`fire_halo` に渡された
    /// `lantern_x`) のスナップショット。パルスリングはこの位置から描く —
    /// 常時リングと違い、パルスは「あの時あの位置で実際に判定した」ことを
    /// 示す演出なので、まとめtick処理後の現在位置ではなく発火時点の位置を
    /// 使わないと、実際に判定したレーンとは違う位置に表示されてしまう。
    pub halo_flash_x: f64,
...
impl EverlightState {
    pub fn new() -> Self {
        ...
        Self {
            ...
            halo_flash_x: lane_center_x(COLUMNS / 2),
            ember: 0,
            camp,
            best_wave: 0,
