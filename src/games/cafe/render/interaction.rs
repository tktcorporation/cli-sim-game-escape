...
use super::super::affinity::{ActionType, CharacterId};
...
pub(super) fn render_character_select(state: &CafeState, f: &mut Frame, area: Rect, click_state: &Rc<RefCell<ClickState>>) {
    ...
    for (i, ch) in unlocked.iter().enumerate() {
        ...
        let stars = aff.map(|a| a.axes.star_rank()).unwrap_or(1);
        ...
        cl.push_clickable(Line::from(vec![
            ...
            Span::styled(format!("  {star_str}"), Style::default().fg(Color::Cyan)),
...
pub(super) fn render_action_select(state: &CafeState, f: &mut Frame, area: Rect, click_state: &Rc<RefCell<ClickState>>, target: CharacterId) {
    ...
    for (action, id) in &actions {
        ...
        // Special requires star rank >= 2
        let special_locked = *action == ActionType::Special && state.affinities.get(&target).map(|a| a.axes.star_rank()).unwrap_or(1) < 2;
        let style = if !can_do || special_locked {
            Style::default().fg(Color::DarkGray)
        } else {
            Style::default().fg(Color::White)
        };
        let gains = action.base_gains();
        cl.push_clickable(Line::from(vec![
        ...
        cl.push(Line::from(Span::styled(format!("   信頼+{} 理解+{} 共感+{}", gains.trust, gains.understanding, gains.empathy), Style::default().fg(Color::DarkGray))));
    }
...
pub(super) fn render_action_result(f: &mut Frame, area: Rect, click_state: &Rc<RefCell<ClickState>>, target: CharacterId, action: ActionType, trust: u32, understanding: u32, empathy: u32) {
    let mut cl = ClickableList::new();
    cl.push(Line::from(""));
    cl.push(Line::from(Span::styled(format!(" {} に「{}」を行った！", target.name(), action.name()), Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD))));
    cl.push(Line::from(""));
...
pub(super) fn render_character_detail(state: &CafeState, f: &mut Frame, area: Rect, click_state: &Rc<RefCell<ClickState>>, target: CharacterId) {
    ...
    if let Some(aff) = state.affinities.get(&target) {
        let stars = "★".repeat(aff.axes.star_rank() as usize);
        cl.push(Line::from(Span::styled(format!(" {stars} 好感度Lv.{}", aff.axes.level()), Style::default().fg(Color::Yellow))));
        cl.push(Line::from(Span::styled(format!(" 次のレベルまで: {}pt", aff.axes.points_to_next_level()), Style::default().fg(Color::DarkGray))));
        cl.push(Line::from(""));
        ...
        cl.push(Line::from(Span::styled(format!("  合計: {}", aff.axes.total()), Style::default().fg(Color::White))));
    }
